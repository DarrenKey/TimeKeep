...
open class WobbleView: UIView {
    ...
    */
    ...
    fileprivate func setUpDisplayLink() {
        displayLink = CADisplayLink(target: self, selector: #selector(WobbleView.displayLinkUpdate(_:)))
        displayLink!.add(to: RunLoop.main, forMode: RunLoop.Mode.default)
        displayLink!.isPaused = true
    }
    
    // MARK: CADisplayLink selector
    @objc internal func displayLinkUpdate(_ sender: CADisplayLink) {
        for behavour in centersAttachments {
            if let tempPlaceholder = centerViews[behavour.vertexIndex!].layer.presentation(){
                behavour.anchorPoint = tempPlaceholder.frame.origin
            }
        }
        
        for behavour in verticesAttachments {
            if let tempPlaceholder = vertexViews[behavour.vertexIndex!].layer.presentation(){
                behavour.anchorPoint = tempPlaceholder.frame.origin
            }
        }
    ...
    fileprivate func addEdge(_ bezierPath: inout UIBezierPath, formerVertex: Int, latterVertex: Int, curved: ViewEdge) {
        ...
        if (curved).boolValue {
            if let tempFormerVertex = vertexViews[formerVertex].layer.presentation(), let tempMidpoint = midpointViews[formerVertex].layer.presentation(), let tempLayer = layer.presentation(), let tempLatterVertex = vertexViews[latterVertex].layer.presentation(){
                let controlPoint = (tempFormerVertex.frame.origin - (tempMidpoint.frame.origin - tempLatterVertex.frame.origin)) - tempLayer.frame.origin
                
                bezierPath.addQuadCurve(to: tempLatterVertex.frame.origin - tempLayer.frame.origin,
